...
function User({ user, index, onDelete, onView, onEdit, onLocationClick }) {
  const handleView = () => {
    onView(user.id);
  };
  ...
  return (
    <div className="grid grid-cols-5 bg-gray-100 items-center border-b border-opacity-30 border-gray-400">
      ...
      <p className="py-2 justify-self-center">{user.name}</p>
      <p className="py-2 justify-self-center">{user.familyName}</p>
      <p className="py-2 justify-self-center">{user.idCardNumber}</p>
